...
# khoi tao
  addi  $t1, $zero, 0 #  $t1:  chi so mang
  la  $a1, array  #  $a1:  dia chi co so cua mang

NhapMang:
# kiem tra so lan lap
  subu  $t2, $t1, $t0 # chi so - so phan tu
  bgez  $t2, KetThucNhap # bgez s,label # nhay ??n nhãn label neu thanh ghi $t2 >=0
...
# nhap so nguyen va luu vao array[i]
  addi  $v0, $zero, 5
  syscall
  sw    $v0, ($a1) # a1 la dia chi cua co so
  ...
  lap:
	...
	# lay gia tri ptu a3
  	...
  	#so sanh a3 voi a0, neu a3 < a0 thì haon doi hai so
  		slt $t5, $a3, $a0 # a3 < a0 neu dúng thì 1 sai thì 0
